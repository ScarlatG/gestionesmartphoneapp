...
//			System.out.println("In tabella App ci sono " + appServiceInstance.listAll().size() + " elementi.");
	...
	private static void testRimozioneSmartphoneCompleta(SmartphoneService smartphoneServiceInstance,
		...
		Smartphone smartphoneReloaded = smartphoneServiceInstance
				.caricaSingoloElementoEagerFetchingApps(smartphoneDaEliminare.getId());
		if (smartphoneReloaded.getApps().size() != 2) {
			throw new RuntimeException("testRimozioneSmartphoneCompleta FALLITO: non sono state aggiunte le app.");
		}

		smartphoneServiceInstance.rimozioneSmartphoneCompleta(smartphoneReloaded.getId());
		Smartphone smartphoneSupposedToBeRemoved = smartphoneServiceInstance
				.caricaSingoloElementoEagerFetchingApps(smartphoneDaEliminare.getId());
		if (smartphoneSupposedToBeRemoved != null) {
			throw new RuntimeException("testRimozioneSmartphoneCompleta FALLITO: rimozione non avvenuta.");
		}

		System.out.println("+++++++++  testRimozioneSmartphoneCompleta FINE  +++++++++");
	}

	private static void testInstallaAppEsistenteASmartphoneEsistente(SmartphoneService smartphoneServiceInstance,
			AppService appServiceInstance) throws Exception {
		...
		Smartphone smartphoneReloaded = smartphoneServiceInstance
				.caricaSingoloElementoEagerFetchingApps(smartphoneSuCuiInstallareLApp.getId());
	...
	private static void testDisinstallaApp(SmartphoneService smartphoneServiceInstance, AppService appServiceInstance)
			throws Exception {
		System.out.println("+++++++++  testInstallaAppEsistenteASmartphoneEsistente INIZIO  +++++++++");
		List<Smartphone> listaSmartphonePresentiInElenco = smartphoneServiceInstance.listAll();
		if (listaSmartphonePresentiInElenco.size() < 1) {
			throw new RuntimeException("testDisinstallaApp FALLITO: non sono presenti smartphone in elenco.");
		}
		...
		Smartphone smartphoneReloaded = smartphoneServiceInstance
				.caricaSingoloElementoEagerFetchingApps(smartphoneSuCuiDisinstallareLApp.getId());
		if (!smartphoneReloaded.getApps().isEmpty()) {
			throw new RuntimeException("testDisinstallaApp FALLITO: non è stata aggiunta l'app.");
		}
